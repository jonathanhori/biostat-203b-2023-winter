...
We think lab and vital measurements will be important for predicting mortality,
and choose to impute missing values here using medians. We experiment with
alternate methods to handle outlier values.

I'm not sure if there's a better way to reuse a set of multiple steps in more
than one recipe, so this code isn't very DRY.
...
Our transformed data looks like this (without outlier transformation): 
```{r}
baked_data <- bake(recipe_with_normalization_and_dummies, new_data = NULL)
# baked_data <- bake(recipe_with_filter_normalization_and_dummies, new_data = NULL)
