...
These files are distributed as `.csv.gz` files instead of `.csv` files because of their size - the `.csv.gz` files are `.csv` files compressed using gzip.
...
We observe that the admissions data has a header, which we remove in the below
calculations.
...
the `-nc` command prevents the file from being redownloaded if already present.
...
output we see above are lines 15 through 20 in the Pride and Prejudice .txt file.
...
Output the architecture of my device. It seems this computer is running AArch64.
```{bash}
arch
```
...
Output all currently logged in users.
```{bash}
who
```

Output all logged in users with the current process they are running.
```{bash}
w
```
