...
***ANSWER***
```{bash}
datafile=~/mimic/core/admissions.csv.gz
...
```{bash}
# Unique patients:
zcat $datafile | awk -F, '{print $1}' | sort | uniq -u | wc -l
```
There are 523,741 rows in the admissions dataset. There are 171,081 unique patients
...
```{bash}
var_names=("admission_type" "admission_location" "insurance" "insurance")
datafile=~/mimic/core/admissions.csv.gz
