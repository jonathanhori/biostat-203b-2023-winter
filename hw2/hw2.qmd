...
system.time(
  labevents_tble <- labevents_tble %>%
    slice(match(itemid, lab_itemid_to_keep)) %>% 
    print(width = Inf)
)
```
...
#### 3. 
```{r, eval=FALSE}
# determine start of icu stay per patient, slice first lab of each type 
#   occurring after admission

# left join icustays by subject id, filter charttime between in/outitme


# labevents_join_logic <- join_by(subject_id,
#                                 # hadm_id,
#                                 between(charttime, intime, outtime))
# 
# labevents_tble <- semi_join(labevents_tble, icustays_tble,
#                             by = join_logic)
