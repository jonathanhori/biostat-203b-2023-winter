...
```
Omitting the header, there are 122,103,667 rows in the `labevents` data. 
...
# labevents_tble %>% distinct(subject_id) %>% nrow()
...
After filtering for the right patients and lab types, we have 6,788,195 rows.
...
#### 3.
We produce similar distribution plots and summary statistics as in Question 5.
Note the below code is not as DRY as it could be (calling `pivot_longer` twice,
for example), but to avoid requiring another package (`magrittr`), 
we avoid using the Tee pipe `%T>%`, which would facilitate calling `ggplot` and 
`summarise` in the same pipe.
...
```{r}
# todo drop repeated los variable
...
mimic_icu_cohort <- icustays_tble %>% 
  ...
  mutate(
    thirty_day_mort = as.integer(
      (deathtime <= intime + days(30)) %>% replace(is.na(.), FALSE)
    )) %>% 
  ungroup() %>% 
  print(width = Inf)
```
...
mimic_icu_cohort %>% 
  ...
  facet_wrap(~lab_type, scales = "free", space = "free_y", ncol = 2) +
  geom_boxplot() +
  labs(
